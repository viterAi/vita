...
function buildPrompt(speakers, chunkText) {
...
function parseSegments(raw, chunkText) {
    const m = raw.match(/\[[\s\S]*\]/);
    if (!m)
        return { segments: [], warning: 'no JSON array in response' };
    let arr;
    try {
        arr = JSON.parse(m[0]);
    }
    catch (err) {
        return { segments: [], warning: `JSON parse: ${err.message.slice(0, 80)}` };
    }
    if (!Array.isArray(arr))
        return { segments: [], warning: 'response was not an array' };
    const out = [];
    let warning = null;
...
export async function diarizeChunk(args) {
    ...
    let resp;
    if (args.logger) {
        resp = await withLLMCallLog(args.logger, {
            model,
...
export async function diarizeMeeting(args) {
    ...
    const out = new Array(args.chunks.length);
    let cursor = 0;
...
//# sourceMappingURL=diarize.js.map