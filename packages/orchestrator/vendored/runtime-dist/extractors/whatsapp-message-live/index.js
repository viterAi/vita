 ...
 */
export async function ingestLiveMessage(db, input) {
    ...
    if (l0Insert.error) {
        if (l0Insert.error.code === '23505') {
            ...
            const { data } = await db
                ...
                .single();
            if (!data)
                throw new Error('l0 idempotency conflict but no existing row found');
            l0Id = data.id;
        }
        else {
