...
const PROMPT = 'Extract all text from this PDF verbatim, preserving structure (headings, ' +
    'lists, tables as markdown tables when possible). Output ONLY the extracted ' +
    'text. Use page separators of the form `\\n\\n--- page N ---\\n\\n` between pages.';
export async function extractPdf(input, ctx) {
    if (!ctx.openrouterApiKey)
        throw new Error('OPENROUTER_API_KEY required for pdf extractor');
    ...
    if (pageRe.test(text)) {
        ...
        for (let i = 1; i < parts.length; i += 2) {
            ...
            if (content)
                segments.push({ start: null, end: null, page: pg, text: content });
        }
    }
    return {
        kind: 'doc_text',
