...
export async function extractAudio(input, ctx) {
    if (!ctx.openrouterApiKey)
        throw new Error('OPENROUTER_API_KEY required for audio extractor');
    ...
    const t0 = Date.now();
    const data = await withLLMCallLog(ctx.logger, {
        model,
