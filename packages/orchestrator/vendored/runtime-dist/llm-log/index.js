 ...
 */
export function createLLMCallLogger(spec) {
    return {
        async start(args) {
            const baseMetadata = {};
            if (spec.source)
                baseMetadata.source = spec.source;
            if (spec.triggerRunId)
                baseMetadata.trigger_run_id = spec.triggerRunId;
            const row = {
        ...
        async finish(id, args) {
            ...
            if (spec.triggerRunId)
                baseMetadata.trigger_run_id = spec.triggerRunId;
            if (args.metadataExtra)
                Object.assign(baseMetadata, args.metadataExtra);
            ...
            if (args.modelUsed != null)
                update.model_used = args.modelUsed;
            if (args.providerName != null)
                update.provider_name = args.providerName;
            if (args.generationId != null)
                update.generation_id = args.generationId;
            if (args.finishReason != null)
                update.finish_reason = args.finishReason;
            if (args.promptTokens != null)
                update.prompt_tokens = args.promptTokens;
            if (args.completionTokens != null)
                update.completion_tokens = args.completionTokens;
            if (args.reasoningTokens != null)
                update.reasoning_tokens = args.reasoningTokens;
            if (args.cachedTokens != null)
                update.cached_tokens = args.cachedTokens;
            if (args.totalTokens != null)
                update.total_tokens = args.totalTokens;
            if (args.costUsd != null) {
                update.cost_usd = args.costUsd;
                update.cost_source = 'caller';
            }
            if (args.generationTimeMs != null)
                update.generation_time_ms = args.generationTimeMs;
            if (args.errorMessage != null)
                update.error_message = args.errorMessage;
            if (args.errorCode != null)
                update.error_code = args.errorCode;
            if (args.rawResponse != null)
                update.raw_response = args.rawResponse;
            ...
            if (typeof m.audio_seconds === 'number')
                update.audio_seconds = m.audio_seconds;
            if (typeof m.audio_format === 'string')
                update.audio_format = m.audio_format;
            if (typeof m.audio_bytes_sent === 'number')
                update.audio_bytes = m.audio_bytes_sent;
            if (typeof m.audio_language === 'string')
                update.audio_language = m.audio_language;
            if (typeof m.n_segments === 'number')
                update.audio_n_segments = m.n_segments;
            if (typeof m.chars === 'number')
                update.output_chars = m.chars;
            if (typeof m.output_kind === 'string')
...
export async function withLLMCallLog(logger, startArgs, fn) {
    const t0 = Date.now();
